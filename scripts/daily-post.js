...
async function callAI(prompt) {
  ...
  if (OR) {
    for (const model of ["openai/gpt-4o-mini", "meta-llama/llama-3.3-70b-instruct:free"]) {
      for (let t = 0; t < 3; t++) {
        try {
          const r = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { Authorization: "Bearer " + OR, "Content-Type": "application/json", "HTTP-Referer": "https://ouniversoanimes.blogspot.com", "X-Title": "O Mundo dos Animes" }, body: JSON.stringify({ model, messages: [{ role: "user", content: prompt }], temperature: 0.85, max_tokens: 3500 }), signal: AbortSignal.timeout(90000) });
  ...
  for (let t = 0; t < 4; t++) {
    try {
      const r = await fetch("https://text.pollinations.ai/openai", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "openai-fast", messages: [{ role: "user", content: prompt }], temperature: 0.8 }), signal: AbortSignal.timeout(90000) });
